...
const petsContract = new web3.eth.Contract(
  contract_abi,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `Pet ${id} has not been claimed yet` });
  }
  ...
  try {
    ...
    const resizedImgArr = new Uint8Array(
      await sharp(imgArr).resize(resizeSize, resizeSize).toBuffer()
    );

    const element =
      attributes.filter((a) => a.trait_type === "Element")[0]?.value || "egg";

    // image creation + composition
    ...
    return res.status(200).json(wallpaperBuffer.toJSON());
  } catch (error) {
